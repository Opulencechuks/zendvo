...
const route_1 = require("@/app/api/transactions/route");
...
function makeRequest(params = {}, userId = "user-123") {
...
describe("Transactions API", () => {
    beforeEach(() => {
        mockDb.query.gifts.findMany.mockResolvedValue(mockGifts);
    ...
    test("400 on invalid type param", async () => {
        const res = await (0, route_1.GET)(makeRequest({ type: "unknown" }));
        expect(res.status).toBe(400);
    });
    test("400 on invalid page param", async () => {
        const res = await (0, route_1.GET)(makeRequest({ page: "0" }));
        expect(res.status).toBe(400);
    });
    test("400 on limit exceeding 100", async () => {
        const res = await (0, route_1.GET)(makeRequest({ limit: "999" }));
        expect(res.status).toBe(400);
    });
    ...
    test("pagination params are respected", async () => {
        await (0, route_1.GET)(makeRequest({ page: "2", limit: "5" }));
        expect(mockDb.query.gifts.findMany).toHaveBeenCalledWith(expect.objectContaining({
            limit: 5,
            offset: 5,
        }));
    });
    test("400 on non-integer page param", async () => {
        const res = await (0, route_1.GET)(makeRequest({ page: "1abc" }));
        expect(res.status).toBe(400);
    });
    test("400 on non-integer limit param", async () => {
        const res = await (0, route_1.GET)(makeRequest({ limit: "10.5" }));
        expect(res.status).toBe(400);
    });
});
