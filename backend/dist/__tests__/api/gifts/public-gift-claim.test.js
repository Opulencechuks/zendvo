...
const route_1 = require("@/app/api/gifts/public/[giftId]/claim/route");
...
jest.mock("@/lib/paystack/api", () => ({
    verifyBankAccount: jest.fn(() => Promise.resolve({ success: true, status: "mock_verified", name: "Verified Recipient" })),
    initiateBankPayout: jest.fn(() => Promise.resolve({ success: true, payoutReference: "payout-ref-123" })),
...
function makeRequest(giftId, body) {
    return new server_1.NextRequest(`http://localhost/api/gifts/public/${giftId}/claim`, {
        method: "POST",
...
describe("POST /api/gifts/public/:giftId/claim", () => {
    ...
    it("should return 400 when recipientId is missing for wallet destination", async () => {
        ...
        db_1.db.query.gifts.findFirst.mockResolvedValue(giftWithoutRecipient);
        const request = makeRequest("gift-123", { destinationType: "wallet" });
        const response = await (0, route_1.POST)(request, {
            params: Promise.resolve({ giftId: "gift-123" }),
        });
