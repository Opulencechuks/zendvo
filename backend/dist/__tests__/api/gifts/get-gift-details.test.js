...
const route_1 = require("@/app/api/gifts/[giftId]/route");
...
function makeRequest(giftId, userId) {
    const headers = {
    ...
    return new server_1.NextRequest(`http://localhost/api/gifts/${giftId}`, {
        method: "GET",
        headers,
    });
}
describe("GET /api/gifts/:giftId", () => {
    ...
    it("should return 200 with full gift details for creator", async () => {
        ...
        const response = await (0, route_1.GET)(makeRequest("gift-123", "sender-123"), {
            params: Promise.resolve({ giftId: "gift-123" }),
        });
        expect(response.status).toBe(200);
    });
    it("should return 401 when unauthenticated", async () => {
        const response = await (0, route_1.GET)(makeRequest("gift-123"), {
            params: Promise.resolve({ giftId: "gift-123" }),
        });
    ...
    it("should return 404 when gift does not exist", async () => {
        ...
        const response = await (0, route_1.GET)(makeRequest("gift-999", "sender-123"), {
    ...
    it("should return 403 when requester is not the gift creator", async () => {
        db_1.db.query.gifts.findFirst.mockResolvedValue(mockGift);
        const response = await (0, route_1.GET)(makeRequest("gift-123", "other-user-000"), {
            params: Promise.resolve({ giftId: "gift-123" }),
        });
