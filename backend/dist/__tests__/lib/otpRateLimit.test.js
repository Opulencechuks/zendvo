...
describe("OTP Rate Limiting", () => {
    ...
    describe("checkOTPRequestRateLimitByUserId", () => {
        it("should allow request when under the limit", async () => {
            ...
            const result = await (0, otpService_1.checkOTPRequestRateLimitByUserId)(mockUserId);
            expect(result.allowed).toBe(true);
            expect(result.remainingRequests).toBe(otpService_1.MAX_OTP_REQUESTS_PER_PHONE - 2 - 1);
    ...
    describe("checkOTPRequestRateLimit", () => {
        it("should allow request when phone number has no associated user", async () => {
            ...
            const result = await (0, otpService_1.checkOTPRequestRateLimit)(mockPhoneNumber);
            expect(result.allowed).toBe(true);
            expect(result.remainingRequests).toBe(otpService_1.MAX_OTP_REQUESTS_PER_PHONE);
        });
    ...
    describe("Rate limit constants", () => {
        it("should have max 4 OTP requests per phone", () => {
            expect(otpService_1.MAX_OTP_REQUESTS_PER_PHONE).toBe(4);
        });
        it("should have 10 minute window", () => {
            expect(otpService_1.OTP_RATE_LIMIT_WINDOW_MS).toBe(10 * 60 * 1000);
        });
    });
});
