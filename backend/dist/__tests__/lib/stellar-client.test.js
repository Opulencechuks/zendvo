...
const mockNativeToScVal = jest.fn((value, options) => ({
    type: options.type,
    value,
}));
...
jest.mock("@stellar/stellar-sdk", () => ({
    ...
    StrKey: {
        isValidContract: jest.fn((value) => value === "CCONTRACTID"),
        isValidEd25519PublicKey: jest.fn((value) => value === "GCONFIGUREDSIGNERPUBKEY"),
    },
    ...
    xdr: {
        ScVal: {
            scvBytes: jest.fn((value) => ({
                bytes: value.toString("hex"),
            })),
        },
    },
}));
const client_1 = require("@/lib/stellar/client");
describe("EscrowContract.create_gift", () => {
    ...
    it("submits create_gift and returns the finalized transaction hash", async () => {
        ...
        const txHash = await client_1.EscrowContract.create_gift({
        ...
        expect(mockContractCall).toHaveBeenCalledWith("create_gift", "sender-scval", { bytes: recipientHash }, { type: "u128", value: "1000" }, { type: "u64", value: "1711549200" });
    ...
    it("fails fast when config is missing", async () => {
        ...
        await expect(client_1.EscrowContract.create_gift({
            sender: "GCONFIGUREDSIGNERPUBKEY",
