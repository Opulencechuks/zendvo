...
const VALID_DESTINATIONS = ["wallet", "bank"];
...
async function POST(request, { params }) {
    try {
        ...
        const destinationType = isValidDestination(destinationTypeRaw)
            ? destinationTypeRaw
            : "wallet";
        if (destinationTypeRaw !== undefined &&
            !isValidDestination(destinationTypeRaw)) {
            return (0, api_utils_1.createProblemDetails)("about:blank", "Bad Request", 400, "destinationType must be either 'wallet' or 'bank'");
        }
        const bankAccountNumber = typeof body.bankAccountNumber === "string"
            ...
            : typeof body.accountNumber === "string"
                ...
                : null;
        const bankCode = typeof body.bankCode === "string" ? body.bankCode.trim() : null;
        ...
        if (gift.status !== "pending_review") {
            if (gift.status === "completed") {
                return (0, api_utils_1.createProblemDetails)("about:blank", "Conflict", 409, "Gift has already been claimed");
            }
            return (0, api_utils_1.createProblemDetails)("about:blank", "Bad Request", 400, `Gift cannot be claimed. Current status: ${gift.status}. Expected: pending_review`);
        }
        ...
        if (destinationType === "wallet") {
            if (!gift.recipientId) {
                return (0, api_utils_1.createProblemDetails)("about:blank", "Bad Request", 400, "Recipient ID is required to claim funds to a Zendvo wallet");
            }
            transactionId = await (0, transactionService_1.processGiftTransaction)({
            ...
            if (!bankAccountNumber || !bankCode) {
                return (0, api_utils_1.createProblemDetails)("about:blank", "Bad Request", 400, "Bank account number and bank code are required for bank payouts");
            }
            const verification = await (0, api_1.verifyBankAccount)(bankAccountNumber, bankCode);
            if (!verification || verification.status !== "mock_verified") {
                return (0, api_utils_1.createProblemDetails)("about:blank", "Bad Request", 400, "Unable to verify bank account details");
            }
            await (0, transactionService_1.processGiftBankPayout)({
                senderId: gift.senderId,
                amount: gift.amount,
                currency: gift.currency,
            });
            const payoutResult = await (0, api_1.initiateBankPayout)({
                bankAccountNumber,
                bankCode,
                amount: gift.amount,
                currency: gift.currency,
                recipientName: gift.recipient?.name || verification.name || "Zendvo Recipient",
            });
            if (!payoutResult.success) {
                return (0, api_utils_1.createProblemDetails)("about:blank", "Bad Gateway", 502, "Unable to initiate bank payout");
            }
        ...
        if (gift.senderId && gift.sender) {
            (0, emailService_1.sendGiftCompletionToSender)(gift.sender.email, gift.sender.name || "Valued Sender", shareLink, gift.amount, gift.currency, gift.recipient?.name || "Gift Recipient").catch((err) => console.error("[GIFT_CLAIM_SENDER_EMAIL_ERROR]", err));
        }
        else if (gift.senderEmail && gift.senderName) {
            (0, emailService_1.sendGiftCompletionToSender)(gift.senderEmail, gift.senderName, shareLink, gift.amount, gift.currency, gift.recipient?.name || "Gift Recipient").catch((err) => console.error("[GIFT_CLAIM_PUBLIC_SENDER_EMAIL_ERROR]", err));
        }
        if (gift.recipient) {
            (0, emailService_1.sendGiftNotificationToRecipient)(gift.recipient.email, gift.recipient.name || "Valued Recipient", gift.senderName || (gift.sender?.name ?? "Someone"), gift.amount, gift.currency, gift.unlockDatetime ?? undefined).catch((err) => console.error("[GIFT_CLAIM_RECIPIENT_EMAIL_ERROR]", err));
        }
        ...
        if (error instanceof Error && error.message.includes("Insufficient balance")) {
            return (0, api_utils_1.createProblemDetails)("about:blank", "Unprocessable Entity", 422, "Insufficient balance to process this claim");
        }
