"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OTP_RATE_LIMIT_WINDOW_MS = exports.MAX_OTP_REQUESTS_PER_PHONE = void 0;
exports.checkOTPRequestRateLimit = checkOTPRequestRateLimit;
exports.checkOTPRequestRateLimitByUserId = checkOTPRequestRateLimitByUserId;
exports.generateOTP = generateOTP;
exports.hashOTP = hashOTP;
exports.verifyOTPHash = verifyOTPHash;
exports.sendOTP = sendOTP;
exports.storeOTP = storeOTP;
exports.verifyOTP = verifyOTP;
exports.cleanupExpiredOTPs = cleanupExpiredOTPs;
exports.storeGiftOTP = storeGiftOTP;
exports.verifyGiftOTP = verifyGiftOTP;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const crypto_1 = __importDefault(require("crypto"));
const db_1 = require("@/lib/db");
const schema_1 = require("@/lib/db/schema");
const drizzle_orm_1 = require("drizzle-orm");
const validation_1 = require("@/lib/validation");
const auditService_1 = require("@/server/services/auditService");
const emailService_1 = require("./emailService");
const SUSPICIOUS_OTP_THRESHOLD = 20;
const IP_TRACKING_WINDOW_MS = 60 * 60 * 1000;
const otpFailuresByIp = new Map();
exports.MAX_OTP_REQUESTS_PER_PHONE = 4;
exports.OTP_RATE_LIMIT_WINDOW_MS = 10 * 60 * 1000;
async function checkOTPRequestRateLimit(phoneNumber) {
    const windowStart = new Date(Date.now() - exports.OTP_RATE_LIMIT_WINDOW_MS);
    const user = await db_1.db.query.users.findFirst({
        where: (0, drizzle_orm_1.eq)(schema_1.users.phoneNumber, phoneNumber),
        columns: { id: true },
    });
    if (!user) {
        return {
            allowed: true,
            remainingRequests: exports.MAX_OTP_REQUESTS_PER_PHONE,
            retryAfterMs: 0,
        };
    }
    const recentOTPs = await db_1.db
        .select({ count: (0, drizzle_orm_1.sql) `count(*)::int` })
        .from(schema_1.emailVerifications)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailVerifications.userId, user.id), (0, drizzle_orm_1.gt)(schema_1.emailVerifications.createdAt, windowStart)));
    const otpCount = recentOTPs[0]?.count ?? 0;
    if (otpCount >= exports.MAX_OTP_REQUESTS_PER_PHONE) {
        const oldestOTP = await db_1.db.query.emailVerifications.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailVerifications.userId, user.id), (0, drizzle_orm_1.gt)(schema_1.emailVerifications.createdAt, windowStart)),
            orderBy: [schema_1.emailVerifications.createdAt],
            columns: { createdAt: true },
        });
        const retryAfterMs = oldestOTP
            ? Math.max(0, exports.OTP_RATE_LIMIT_WINDOW_MS -
                (Date.now() - new Date(oldestOTP.createdAt).getTime()))
            : exports.OTP_RATE_LIMIT_WINDOW_MS;
        return {
            allowed: false,
            remainingRequests: 0,
            retryAfterMs,
            message: `Too many OTP requests. Please wait ${Math.ceil(retryAfterMs / 60000)} minutes before requesting a new code.`,
        };
    }
    return {
        allowed: true,
        remainingRequests: exports.MAX_OTP_REQUESTS_PER_PHONE - otpCount - 1,
        retryAfterMs: 0,
    };
}
async function checkOTPRequestRateLimitByUserId(userId) {
    const windowStart = new Date(Date.now() - exports.OTP_RATE_LIMIT_WINDOW_MS);
    const recentOTPs = await db_1.db
        .select({ count: (0, drizzle_orm_1.sql) `count(*)::int` })
        .from(schema_1.emailVerifications)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailVerifications.userId, userId), (0, drizzle_orm_1.gt)(schema_1.emailVerifications.createdAt, windowStart)));
    const otpCount = recentOTPs[0]?.count ?? 0;
    if (otpCount >= exports.MAX_OTP_REQUESTS_PER_PHONE) {
        const oldestOTP = await db_1.db.query.emailVerifications.findFirst({
            where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailVerifications.userId, userId), (0, drizzle_orm_1.gt)(schema_1.emailVerifications.createdAt, windowStart)),
            orderBy: [schema_1.emailVerifications.createdAt],
            columns: { createdAt: true },
        });
        const retryAfterMs = oldestOTP
            ? Math.max(0, exports.OTP_RATE_LIMIT_WINDOW_MS -
                (Date.now() - new Date(oldestOTP.createdAt).getTime()))
            : exports.OTP_RATE_LIMIT_WINDOW_MS;
        return {
            allowed: false,
            remainingRequests: 0,
            retryAfterMs,
            message: `Too many OTP requests. Please wait ${Math.ceil(retryAfterMs / 60000)} minutes before requesting a new code.`,
        };
    }
    return {
        allowed: true,
        remainingRequests: exports.MAX_OTP_REQUESTS_PER_PHONE - otpCount - 1,
        retryAfterMs: 0,
    };
}
function generateOTP() {
    return crypto_1.default.randomInt(100000, 999999).toString();
}
function hashOTP(otp) {
    const salt = crypto_1.default.randomBytes(16).toString("hex");
    const hash = crypto_1.default.createHmac("sha256", salt).update(otp).digest("hex");
    return { salt, hash };
}
function verifyOTPHash(otp, storedHash, salt) {
    const hash = crypto_1.default.createHmac("sha256", salt).update(otp).digest("hex");
    if (hash.length !== storedHash.length) {
        return false;
    }
    return crypto_1.default.timingSafeEqual(Buffer.from(hash), Buffer.from(storedHash));
}
async function sendOTP(phoneNumber) {
    try {
        if (!(0, validation_1.validateE164PhoneNumber)(phoneNumber)) {
            return {
                success: false,
                message: "Invalid phone number format. Please use E.164 format (e.g., +2348123456789)",
                error: "INVALID_PHONE_FORMAT",
                detail: "INVALID_PHONE_FORMAT",
            };
        }
        const sanitizedPhone = (0, validation_1.sanitizePhoneNumber)(phoneNumber);
        const user = await db_1.db.query.users.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.users.phoneNumber, sanitizedPhone),
        });
        if (!user) {
            return {
                success: false,
                message: "User not found with this phone number",
                error: "USER_NOT_FOUND",
                detail: "USER_NOT_FOUND",
            };
        }
        if (user.status === "suspended") {
            return {
                success: false,
                message: "Account suspended",
                error: "ACCOUNT_SUSPENDED",
                detail: "ACCOUNT_SUSPENDED",
            };
        }
        const otp = generateOTP();
        await storeOTP(user.id, otp);
        console.log(`[SMS_OTP] Phone: ${sanitizedPhone}, OTP: ${otp}`);
        const smsResult = await sendSMSViaProvider(sanitizedPhone, `Your Zendvo verification code is: ${otp}. Valid for 10 minutes.`);
        if (!smsResult.success) {
            console.error("Failed to send OTP SMS:", smsResult.error);
            return {
                success: false,
                message: "Failed to send OTP SMS",
                error: "SMS_SEND_FAILED",
                detail: "SMS_SEND_FAILED",
            };
        }
        console.log(`[AUDIT] SMS OTP sent to ${sanitizedPhone} for user ${user.id}`);
        return {
            success: true,
            message: "OTP sent successfully via SMS"
        };
    }
    catch (error) {
        console.error("[SEND_PHONE_OTP_ERROR]", error);
        return {
            success: false,
            message: "Internal server error",
            error: "INTERNAL_ERROR",
            detail: "INTERNAL_ERROR",
        };
    }
}
async function sendSMSViaProvider(phoneNumber, message) {
    try {
        console.log(`[MOCK_SMS] To: ${phoneNumber}, Message: ${message}`);
        return { success: true };
    }
    catch (error) {
        return { success: false, error: error instanceof Error ? error.message : "Unknown SMS error" };
    }
}
async function storeOTP(userId, otp) {
    const { salt, hash } = hashOTP(otp);
    const storedValue = `${salt}:${hash}`;
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
    await db_1.db
        .update(schema_1.emailVerifications)
        .set({ isUsed: true })
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailVerifications.userId, userId), (0, drizzle_orm_1.eq)(schema_1.emailVerifications.isUsed, false)));
    (0, auditService_1.logOTPEvent)(auditService_1.AuditEventType.OTP_GENERATED, userId);
    const [newVerification] = await db_1.db
        .insert(schema_1.emailVerifications)
        .values({
        userId,
        otpHash: storedValue,
        expiresAt,
        attempts: 0,
        isUsed: false,
    })
        .returning();
    await db_1.db
        .update(schema_1.users)
        .set({ lastOtpSentAt: new Date() })
        .where((0, drizzle_orm_1.eq)(schema_1.users.id, userId));
    return newVerification;
}
async function verifyOTP(userId, otp, ipAddress) {
    const verification = await db_1.db.query.emailVerifications.findFirst({
        where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.emailVerifications.userId, userId), (0, drizzle_orm_1.eq)(schema_1.emailVerifications.isUsed, false)),
        orderBy: [(0, drizzle_orm_1.desc)(schema_1.emailVerifications.createdAt)],
    });
    if (!verification) {
        return {
            success: false,
            message: "No verification code found. Please request a new one.",
            detail: "NO_VERIFICATION_FOUND",
        };
    }
    if (new Date() > verification.expiresAt) {
        return {
            success: false,
            message: "Verification code has expired. Please request a new one.",
            detail: "VERIFICATION_EXPIRED",
        };
    }
    const user = await db_1.db.query.users.findFirst({
        where: (0, drizzle_orm_1.eq)(schema_1.users.id, userId),
    });
    if (user && user.lockUntil && new Date() < user.lockUntil) {
        return {
            success: false,
            message: "Account is temporarily locked. Please try again later.",
            locked: true,
            detail: "ACCOUNT_LOCKED",
        };
    }
    if (verification.attempts >= 5) {
        return {
            success: false,
            message: "Maximum attempts exceeded. Account is locked.",
            locked: true,
            detail: "MAX_ATTEMPTS_EXCEEDED",
        };
    }
    let isValid = false;
    const storedHash = verification.otpHash;
    if (storedHash.includes(":")) {
        const [salt, hash] = storedHash.split(":");
        isValid = verifyOTPHash(otp, hash, salt);
    }
    else {
        isValid = await bcryptjs_1.default.compare(otp, storedHash);
    }
    if (!isValid) {
        const newAttempts = verification.attempts + 1;
        await db_1.db
            .update(schema_1.emailVerifications)
            .set({ attempts: newAttempts })
            .where((0, drizzle_orm_1.eq)(schema_1.emailVerifications.id, verification.id));
        const now = new Date();
        const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
        let cumulativeFailures = (user?.otpFailedAttempts || 0) + 1;
        let windowStart = user?.otpAttemptsWindowStart;
        if (!windowStart || windowStart < oneHourAgo) {
            cumulativeFailures = 1;
            windowStart = now;
        }
        await db_1.db
            .update(schema_1.users)
            .set({
            otpFailedAttempts: cumulativeFailures,
            otpAttemptsWindowStart: windowStart,
        })
            .where((0, drizzle_orm_1.eq)(schema_1.users.id, userId));
        if (ipAddress) {
            const now = Date.now();
            let state = otpFailuresByIp.get(ipAddress);
            if (state && now - state.lastAttempt > IP_TRACKING_WINDOW_MS) {
                otpFailuresByIp.delete(ipAddress);
                state = undefined;
            }
            if (!state) {
                state = {
                    count: 0,
                    userIds: new Set(),
                    phoneNumbers: new Set(),
                    lastAttempt: now,
                };
            }
            state.count++;
            state.lastAttempt = now;
            state.userIds.add(userId);
            if (user?.phoneNumber)
                state.phoneNumbers.add(user.phoneNumber);
            otpFailuresByIp.set(ipAddress, state);
            if (state.count === SUSPICIOUS_OTP_THRESHOLD) {
                (0, emailService_1.sendAdminAlert)({
                    userIds: Array.from(state.userIds),
                    ips: [ipAddress],
                    phoneNumbers: Array.from(state.phoneNumbers),
                    failureCount: state.count,
                }).catch((err) => console.error("[ADMIN_ALERT_ERROR]", err));
            }
        }
        (0, auditService_1.logOTPEvent)(auditService_1.AuditEventType.OTP_VERIFIED_FAILED, userId, {
            attemptNumber: newAttempts,
            cumulativeFailures,
            remainingAttempts: 5 - newAttempts,
        });
        if (cumulativeFailures >= 10) {
            const lockUntil = new Date(now.getTime() + 24 * 60 * 60 * 1000);
            await db_1.db
                .update(schema_1.users)
                .set({
                lockUntil,
                otpFailedAttempts: 0,
                otpAttemptsWindowStart: null,
            })
                .where((0, drizzle_orm_1.eq)(schema_1.users.id, userId));
            (0, auditService_1.logOTPEvent)(auditService_1.AuditEventType.ACCOUNT_LOCKED_10_ATTEMPTS, userId, {
                lockDuration: "24 hours",
                cumulativeFailures,
                reason: "10 failed OTP attempts within 1 hour",
            });
            return {
                success: false,
                message: "Account locked for 24 hours due to repeated failed attempts. Please contact support if you need assistance.",
                locked: true,
                shouldSendAlert: true,
                lockDuration: "24 hours",
                detail: "ACCOUNT_LOCKED_24_HOURS",
            };
        }
        if (newAttempts >= 5) {
            const lockUntil = new Date(now.getTime() + 30 * 60 * 1000);
            await db_1.db.update(schema_1.users).set({ lockUntil }).where((0, drizzle_orm_1.eq)(schema_1.users.id, userId));
            (0, auditService_1.logOTPEvent)(auditService_1.AuditEventType.ACCOUNT_LOCKED_5_ATTEMPTS, userId, {
                lockDuration: "30 minutes",
                attemptNumber: newAttempts,
                reason: "5 failed attempts on current OTP",
            });
            return {
                success: false,
                message: "Maximum attempts exceeded. Account locked for 30 minutes.",
                locked: true,
                shouldSendAlert: true,
                lockDuration: "30 minutes",
                detail: "ACCOUNT_LOCKED_30_MINUTES",
            };
        }
        const remainingAttempts = 5 - newAttempts;
        return {
            success: false,
            message: `Invalid verification code. ${remainingAttempts} attempts remaining.`,
            remainingAttempts,
            detail: "INVALID_VERIFICATION_CODE",
        };
    }
    await db_1.db
        .delete(schema_1.emailVerifications)
        .where((0, drizzle_orm_1.eq)(schema_1.emailVerifications.id, verification.id));
    await db_1.db
        .update(schema_1.users)
        .set({
        status: "active",
        lockUntil: null,
        loginAttempts: 0,
        otpFailedAttempts: 0,
        otpAttemptsWindowStart: null,
        isPhoneVerified: true,
    })
        .where((0, drizzle_orm_1.eq)(schema_1.users.id, userId));
    (0, auditService_1.logOTPEvent)(auditService_1.AuditEventType.OTP_VERIFIED_SUCCESS, userId);
    return { success: true, message: "Email verified successfully!" };
}
async function cleanupExpiredOTPs() {
    const result = await db_1.db
        .delete(schema_1.emailVerifications)
        .where((0, drizzle_orm_1.lt)(schema_1.emailVerifications.expiresAt, new Date()))
        .returning();
    return result.length;
}
async function storeGiftOTP(giftId, otp) {
    const saltRounds = 10;
    const otpHash = await bcryptjs_1.default.hash(otp, saltRounds);
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000);
    return await db_1.db
        .update(schema_1.gifts)
        .set({
        otpHash,
        otpExpiresAt: expiresAt,
        otpAttempts: 0,
    })
        .where((0, drizzle_orm_1.eq)(schema_1.gifts.id, giftId))
        .returning();
}
const MAX_GIFT_OTP_ATTEMPTS = 5;
async function verifyGiftOTP(gift, otp) {
    if (!gift.otpHash || !gift.otpExpiresAt) {
        return {
            success: false,
            message: "No verification code found for this gift.",
            detail: "NO_GIFT_VERIFICATION_FOUND",
        };
    }
    if (gift.otpAttempts >= MAX_GIFT_OTP_ATTEMPTS) {
        (0, auditService_1.logGiftOTPEvent)(auditService_1.AuditEventType.GIFT_OTP_LOCKED, gift.id, {
            attempts: gift.otpAttempts,
        });
        return {
            success: false,
            message: "Maximum attempts exceeded. This gift has been locked.",
            locked: true,
            detail: "GIFT_LOCKED",
        };
    }
    if (new Date() > gift.otpExpiresAt) {
        return {
            success: false,
            message: "Verification code has expired. Please request a new one.",
            detail: "GIFT_VERIFICATION_EXPIRED",
        };
    }
    const isValid = await bcryptjs_1.default.compare(otp, gift.otpHash);
    if (!isValid) {
        const newAttempts = gift.otpAttempts + 1;
        await db_1.db
            .update(schema_1.gifts)
            .set({ otpAttempts: newAttempts })
            .where((0, drizzle_orm_1.eq)(schema_1.gifts.id, gift.id));
        (0, auditService_1.logGiftOTPEvent)(auditService_1.AuditEventType.GIFT_OTP_FAILED, gift.id, {
            attemptNumber: newAttempts,
            remainingAttempts: MAX_GIFT_OTP_ATTEMPTS - newAttempts,
        });
        const remainingAttempts = MAX_GIFT_OTP_ATTEMPTS - newAttempts;
        if (remainingAttempts <= 0) {
            (0, auditService_1.logGiftOTPEvent)(auditService_1.AuditEventType.GIFT_OTP_LOCKED, gift.id, {
                attempts: newAttempts,
                reason: "Maximum attempts exceeded",
            });
        }
        return {
            success: false,
            message: `Invalid verification code. ${remainingAttempts} attempt${remainingAttempts !== 1 ? "s" : ""} remaining.`,
            remainingAttempts,
            locked: remainingAttempts <= 0,
            detail: "INVALID_GIFT_VERIFICATION_CODE",
        };
    }
    await db_1.db
        .update(schema_1.gifts)
        .set({
        status: "otp_verified",
        otpHash: null,
        otpExpiresAt: null,
        otpAttempts: 0,
    })
        .where((0, drizzle_orm_1.eq)(schema_1.gifts.id, gift.id));
    return { success: true, message: "Gift OTP verified successfully!" };
}
